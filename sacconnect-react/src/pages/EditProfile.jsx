...
export default function EditProfile() {
  ...
  const hasExistingProfile = !!(profile && (profile.major || profile.bio || profile.Age != null));
      ...
      <div className="w-full max-w-2xl bg-ss-card border border-white/10 rounded-[2.5rem] p-10 shadow-2xl relative">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-black uppercase tracking-tighter">
            {hasExistingProfile ? (
              <>Edit Your <span className="text-ss-gold">Profile</span></>
            ) : (
              <>Complete Your <span className="text-ss-gold">Profile</span></>
            )}
          </h1>
          <p className="text-gray-500">
            {hasExistingProfile ? "Update your Hornet profile details" : "Tell other Hornets a bit about yourself"}
          </p>
        </div>

        {loadingProfile && <p className="text-gray-500 text-sm text-center mb-6">Loading profile...</p>}
        ...
        <form action={submitAction} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <input name="major" defaultValue={profile?.major || ''} placeholder="Computer Engineering" required className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-ss-gold outline-none transition-all" />
          </div>

          <div className="space-y-2">
            <label className="text-xs font-bold text-ss-gold uppercase ml-1">Age</label>
            <input name="age" type="number" defaultValue={profile?.Age ?? ''} placeholder="21" className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-ss-gold outline-none transition-all" />
          </div>

          <div className="md:col-span-2 space-y-2">
            <label className="text-xs font-bold text-ss-gold uppercase ml-1">Short Bio</label>
            <textarea name="bio" rows="3" defaultValue={profile?.bio || ''} placeholder="I love building React apps and drinking Dutch Bros..." className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-ss-gold outline-none transition-all resize-none"></textarea>
          </div>

          <div className="md:col-span-2 space-y-2">
            <label className="text-xs font-bold text-ss-gold uppercase ml-1">Interests (Comma separated)</label>
            <input name="interests" defaultValue={profile?.interests?.join(', ') || ''} placeholder="Coding, Gym, Overwatch, Sac State" className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-white focus:border-ss-gold outline-none transition-all" />
          </div>
